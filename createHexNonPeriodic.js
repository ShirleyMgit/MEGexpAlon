function createHexNonPer(){
	...
	var nb;
	var n0=graph.nRow*graph.nCol;//graph.nCol-how many in each row (number of col), graph.nRow- How many in each col (number of rows)
	var Ar=[[]];
	for(a=0;a<n0;a++){
		b=0;
		a1 = a+1;
		rn1a = rem(a1,graph.nCol);
		if(rn1a>0){
			coln =rn1a;
		}else{
			coln = graph.nCol;
		}
		rown = Math.ceil(a1/graph.nCol);
		isEv = rem(rown,2);
		if (coln<graph.nCol&&coln>1&&rown>1&&rown<graph.nRow){
			nb = nb0;
		}else{
			if(coln==1){
				if(isEv==0){//even
		...
		if(rown<graph.nRow){
			Ar[a][b] = a+graph.nCol;//up
			b=b+1;
			if(isEv!=0){
				if(coln<graph.nCol){//up and right
					Ar[a][b] = a+graph.nCol+1;
					b=b+1;
				}
			}else{
		...
		if(rown>1){
			Ar[a][b] = a-graph.nCol;//down
			b=b+1;
			if(isEv!=0){
				if(coln<graph.nCol){
					Ar[a][b] = a-graph.nCol+1;//down and right
					b=b+1;
				}
			}else{
				if(coln>1){
					Ar[a][b] = a-graph.nCol-1;//down and left
					b=b+1;
				}
			}
		}
